...
class GUMP {
    ...
    public static function is_valid(array $data, array $validators) {
        ...
        return $gump->run($data) === false ? $gump->get_readable_errors(false) : true;
    }
    ...
    protected function validate_required($field, $input, $param = NULL) {
        if(isset($input[$field]) && !empty($input[$field])) return;
        return array('field' => $field,
                     'value' => NULL,
                     'rule'  => __FUNCTION__,
                     'param' => $param);
    ...
    protected function validate_max_len($field, $input, $param = NULL) {
        ...
        if(function_exists('mb_strlen'))
            if(mb_strlen($input[$field]) <= (int)$param) return;
        else
            if(strlen($input[$field]) <= (int)$param) return;
    ...
    protected function validate_alpha($field, $input, $param = NULL)
    {
        ...
        if(!preg_match("/^([a-zÃ€Ã�Ã‚ÃƒÃ„Ã…Ã‡ÃˆÃ‰ÃŠÃ‹ÃŒÃ�ÃŽÃ�Ã’Ã“Ã”Ã•Ã–Ã™ÃšÃ›ÃœÃ�Ã Ã¡Ã¢Ã£Ã¤Ã¥Ã§Ã¨Ã©ÃªÃ«Ã¬Ã­Ã®Ã¯Ã°Ã²Ã³Ã´ÃµÃ¶Ã¹ÃºÃ»Ã¼Ã½Ã¿])+$/i", $input[$field]) !== FALSE)
        {
            return array(
                'field' => $field,
                'value' => $input[$field],
    ...
    protected function validate_alpha_numeric($field, $input, $param = NULL)
    {
        ...
        if(!preg_match("/^([a-z0-9Ã€Ã�Ã‚ÃƒÃ„Ã…Ã‡ÃˆÃ‰ÃŠÃ‹ÃŒÃ�ÃŽÃ�Ã’Ã“Ã”Ã•Ã–Ã™ÃšÃ›ÃœÃ�Ã Ã¡Ã¢Ã£Ã¤Ã¥Ã§Ã¨Ã©ÃªÃ«Ã¬Ã­Ã®Ã¯Ã°Ã²Ã³Ã´ÃµÃ¶Ã¹ÃºÃ»Ã¼Ã½Ã¿])+$/i", $input[$field]) !== FALSE)
        {
            return array(
                'field' => $field,
                'value' => $input[$field],
    ...
    protected function validate_alpha_dash($field, $input, $param = NULL)
    {
        ...
        if(!preg_match("/^([a-z0-9Ã€Ã�Ã‚ÃƒÃ„Ã…Ã‡ÃˆÃ‰ÃŠÃ‹ÃŒÃ�ÃŽÃ�Ã’Ã“Ã”Ã•Ã–Ã™ÃšÃ›ÃœÃ�Ã Ã¡Ã¢Ã£Ã¤Ã¥Ã§Ã¨Ã©ÃªÃ«Ã¬Ã­Ã®Ã¯Ã°Ã²Ã³Ã´ÃµÃ¶Ã¹ÃºÃ»Ã¼Ã½Ã¿_-])+$/i", $input[$field]) !== FALSE)
        {
            return array(
                'field' => $field,
                'value' => $input[$field],
    ...
    protected function validate_valid_ipv4($field, $input, $param = NULL) {
        ...
        if(!filter_var($input[$field], FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) { // it passes
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule'  => __FUNCTION__,
    ...
    protected function validate_valid_name($field, $input, $param = NULL) {
        ...
        if(!preg_match("/^([a-zÃ€Ã�Ã‚ÃƒÃ„Ã…Ã‡ÃˆÃ‰ÃŠÃ‹ÃŒÃ�ÃŽÃ�Ã’Ã“Ã”Ã•Ã–Ã™ÃšÃ›ÃœÃ�Ã Ã¡Ã¢Ã£Ã¤Ã¥Ã§Ã¨Ã©ÃªÃ«Ã¬Ã­Ã®Ã¯Ã±Ã°Ã²Ã³Ã´ÃµÃ¶Ã¹ÃºÃ»Ã¼Ã½Ã¿ '-])+$/i", $input[$field]) !== FALSE) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule'  => __FUNCTION__,
